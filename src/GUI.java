...
public class GUI {
	static String FILE_PATH = "/home/katerynasavina/Documents/tpks1.txt";
	public static LSAParser parser;
    static JButton ok = new JButton("OK");
    static JMenuBar menu = new JMenuBar();
    final static JTextArea input = new JTextArea(2, 30);
    final static JTextArea output = new JTextArea();
	...
	protected static void ok() {
		try {
			...
			LSAParser.lsa = new ArrayList<String> (Arrays.asList(parser.s.split("\\s* \\s*")));
	...
	protected static void addComps(final Container pane){
	...
	static void generateLSA(){
				...
				for(int j = 0; j < LSAParser.matrix.length; j++) {
					if(LSAParser.matrix[nodes.get(i).n][j] == 1){
						for(int k = 0; k < nodes.size(); k++){
							if(nodes.get(k).n == j)
								nodes.get(i).setNext(nodes.get(k));
						}
					}
				}
			}
			if(nodes.get(i).name.equals("x")) {
				for(int j = 0; j < LSAParser.matrix.length; j++) {
					...
					if(LSAParser.matrix[nodes.get(i).n][j] == 2) {
						for(int k = 0; k < nodes.size(); k++) {
							if(nodes.get(k).n == j)
								((Condition) nodes.get(i)).setFalseWay(nodes.get(k));
						}
					}
				}
			}
		...
		for(int i = 0; i < nodes.size(); i++) {
			if(nodes.get(i).getNext() != null){
				output.setText(output.getText() + "\n" + nodes.get(i).name + nodes.get(i).ID + " --->  " + nodes.get(i).getNext().name + nodes.get(i).getNext().ID);
			}
		}
	}
	
	static void reachabilityMatrix(int[][] matrix){
		int[][] E1 = matrix.clone();
